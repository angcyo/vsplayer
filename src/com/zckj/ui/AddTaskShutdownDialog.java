 ...
 */
public class AddTaskShutdownDialog extends BaseTaskDialog implements
	...
	public AddTaskShutdownDialog(Context context, int theme,
			ShutdownTaskRecord task, OnTaskAdd listener) {
		super(context, theme);
		this.taskAddListener = listener;
		this.taskRecord = task;
	}

	protected AddTaskShutdownDialog(Context context, boolean cancelable,
	...
	public void onClick(View v) {
		...
		if (id == R.id.id_bt_volume_hour_add) {
			...
			if (taskRecord == null) {
				...
				this.cancel();
				Util.showPostMsg(getContext(), "执行完成");
			}
	...
	void asyncSaveTask(final boolean cancel) {
		new AsyncRun() {
			@Override
			public void doBack() {
				ShutdownHelper.save(getHour(), getMinute(), getTaskType(),
